...
public class Shooter {
    ...
    private static float cameraHeight = 21.0f;
    private static float cameraAngle = 45.5f;
    static float angleError = -0.87f;

    static float calculatedVelocity = 0.0f;
    static float calculatedAngle = 0.000f;
    public static float shootingCoefficient = 2.582f;
    ...
    public double dumpShotSpeed = 0.41;
    ...
    public static float getXDistanceFromCenterOfHub(double verticalAngle) {
        return (float) (((y - cameraHeight) / Math.tan(Math.toRadians(verticalAngle +
                ...
                + (distanceFromTapeToCenterOfHub - 18.0f) + horizontalDistanceFromLimeLightToShooter);
    }
    ...
    public boolean centerRobotOnTarget() {
        if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) < -20.0) {
            ...
            return false;
        } else if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) < -10.0) {
            if (DriverStation.isTeleop()) {
                Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0),
                        -0.125, true);
            } else {
            ...
            return false;
        } else if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) < -6.0) {
            if (DriverStation.isTeleop()) {
                Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0),
                        -0.125, true);
            } else {
            ...
            return false;
        } else if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) < -3.0) {
            if (DriverStation.isTeleop()) {
                Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0),
                        -0.125, true);
            } else {
    ...
    public void shoot() {
        ...
        if (shooting && !dumpShot) {
            ...
            if (Math.abs(calculatedAngle - tempAngle) > 0.004) {
                ...
                if ((masterShooterMotor.getSelectedSensorVelocity() > (shooterWheelLinearVelocityToMotorVelocity(calculatedVelocity) * 1.0175)) && setHoodYet) {
    ...
    public void dumpShotLEDs() {
        double currentDistance = getXDistanceFromCenterOfHub(NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.0));
    ...
    public void dumpShot() { // 92 inches according to distance method
        ...
        if (dumpShot && !shooting) {
            ...
            if ((masterShooterMotor.getSelectedSensorVelocity() > (dumpShotSpeed * 22068.97 * 1.009)) && setHoodYet) {
                Robot.intake.indexerMotor.set(-0.4);
            } else {
                Robot.intake.indexerMotor.set(0.0);
            }
